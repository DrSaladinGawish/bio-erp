...
class TestCreateLedgerEntry:
    ...
    async def test_create_returns_200_or_201(
        self, client: AsyncClient, auth_headers: dict
    ):
        resp = await client.post(
            self.ENDPOINT, json=SAMPLE_PAYLOAD, headers=auth_headers
        )
        assert resp.status_code in (200, 201)

    async def test_create_missing_field_returns_422(
        self, client: AsyncClient, auth_headers: dict
    ):
...
class TestUpdateDeleteLedgerEntry:
    ...
    async def test_update_nonexistent_returns_404(
        ...
        resp = await client.put(
            f"{self.BASE}/99999", json=SAMPLE_UPDATE, headers=auth_headers
        )
...
class TestPagination:
    ...
    async def test_pagination_page_size_respected(
        ...
        resp = await client.get(
            self.ENDPOINT, params={"page": 1, "page_size": 10}, headers=auth_headers
        )
        assert resp.status_code == 200

    async def test_pagination_default_size(
        self, client: AsyncClient, auth_headers: dict
    ):
    ...
    async def test_pagination_out_of_range_returns_empty(
        ...
        resp = await client.get(
            self.ENDPOINT, params={"page": 9999, "page_size": 50}, headers=auth_headers
        )
        assert resp.status_code == 200

    async def test_pagination_contains_total_count(
        self, client: AsyncClient, auth_headers: dict
    ):
        resp = await client.get(
            self.ENDPOINT, params={"page": 1, "page_size": 10}, headers=auth_headers
...
class TestDateFiltering:
    ...
    async def test_date_filter_out_of_range_returns_empty(
        self, client: AsyncClient, auth_headers: dict
    ):
        resp = await client.get(
            self.ENDPOINT,
    ...
    async def test_date_filter_malformed_returns_422(
        self, client: AsyncClient, auth_headers: dict
    ):
        resp = await client.get(
            self.ENDPOINT,
    ...
    async def test_date_filter_date_from_only(
        self, client: AsyncClient, auth_headers: dict
    ):
        resp = await client.get(
            self.ENDPOINT,
