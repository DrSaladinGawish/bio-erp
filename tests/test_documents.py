...
def test_schemas_importable():
    from app.schemas.documents import (
        IngestRequest, IngestResponse, VerifyRequest, VerifyResult,
        LinkRequest, DocumentOut, DocumentListResponse,
        DocumentModuleOut, DocumentModuleList, IngestResultItem,
    )
...
def test_document_model_fields():
    ...
    doc = SupportingDocument(
        module_name="Test", function_name="Test",
        transaction_table="test_table", transaction_id="123",
        archive_path="/tmp/test.pdf", file_hash_sha256="a"*64,
        file_size_bytes=1024, file_name="test.pdf", file_ext="pdf",
    )
...
def test_document_module_fields():
    ...
    dm = DocumentModule(
        module_name="Bank", function_name="Statements",
        transaction_table="bnk_transactions", description="Monthly bank statements",
...
def test_document_module_repr():
    from app.models.documents import DocumentModule
    dm = DocumentModule(module_name="Bank", function_name="Statements", transaction_table="bnk_transactions")
    r = repr(dm)
...
def test_supporting_document_repr():
    from app.models.documents import SupportingDocument
    doc = SupportingDocument(module_name="Bank", function_name="Statements", file_name="test.pdf", transaction_table="x", transaction_id="1", archive_path="/p", file_hash_sha256="a"*64, file_size_bytes=1, file_ext="pdf")
    r = repr(doc)
    assert "Bank" in r
...
def test_ingest_response_fields():
    ...
    resp = IngestResponse(
        total_files_found=10, ingested=8, linked=7,
        orphaned=1, errors=0, bytes_copied=50000,
        manifest=[
            IngestResultItem(file="Bank/test.pdf", status="linked", transaction_id="123"),
        ],
    )
...
def test_document_out():
    ...
    doc = DocumentOut(
        ...
        module_name="Bank", function_name="Statements",
        transaction_table="bnk_transactions", transaction_id="42",
        archive_path="/tmp/test.pdf", file_hash_sha256="a"*64,
        file_size_bytes=1024, file_name="test.pdf", file_ext="pdf",
        ingested_at=datetime.utcnow(), status="linked",
    )
