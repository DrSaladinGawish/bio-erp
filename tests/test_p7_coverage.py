...
class TestAIBridge:
    ...
    async def test_ai_query_empty_body_returns_422(
...
class TestBudgetLifecycle:
    ...
    async def test_snapshot_detail_not_500(
        self, client: AsyncClient, auth_headers: dict
    ):
...
class TestHTMXDashboard:
    ...
    async def test_get_auth_not_500(
        self, path: str, client: AsyncClient, auth_headers: dict
    ):
        resp = await client.get(path, headers=auth_headers)
        assert resp.status_code < 500, f"{path} returned {resp.status_code}"
    ...
    async def test_ai_query_no_auth(self, client: AsyncClient):
        resp = await client.post(
            "/api/v1/dashboard/ai-query", json={"question": "test"}
        )
        assert resp.status_code in (401, 403)
...
class TestReports:
    ...
    async def test_create_executive_not_500(
        self, client: AsyncClient, auth_headers: dict
    ):
        resp = await client.post(
            "/api/v1/reports/executive", json={}, headers=auth_headers
        )
    ...
    async def test_export_invalid_format_422(
        ...
        resp = await client.get(
            "/api/v1/reports/export?format=xml", headers=auth_headers
        )
        assert resp.status_code == 422
...
class TestSystem:
    ...
    @pytest.mark.parametrize("path", ENDPOINTS_POST)
    async def test_post_not_500(
        self, path: str, client: AsyncClient, auth_headers: dict
    ):
        if path == "/email/test":
            resp = await client.post(
                path, params={"recipient": "test@test.com"}, headers=auth_headers
            )
        else:
...
class TestBudgetExtra:
    ...
    async def test_delete_budget_line_not_500(
        self, client: AsyncClient, auth_headers: dict
    ):
...
class TestEventsExtra:
    async def test_receive_event_not_500(self, client: AsyncClient, auth_headers: dict):
        resp = await client.post(
            "/api/v1/events/receive", json={}, headers=auth_headers
        )
...
class TestAdminExtra:
    ...
    async def test_set_user_roles_not_500(
        self, client: AsyncClient, auth_headers: dict
    ):
        resp = await client.put(
            "/api/v1/admin/users/1/roles", json={"role_ids": [1]}, headers=auth_headers
        )
        assert resp.status_code in (200, 404, 422, 403)

    async def test_admin_post_no_auth(self, client: AsyncClient):
        resp = await client.post(
            "/api/v1/admin/roles", json={"name": "test", "code": "test"}
        )
        assert resp.status_code in (401, 403)
...
class TestCurrencyExtra:
    ...
    async def test_update_rate_not_500(self, client: AsyncClient, auth_headers: dict):
        resp = await client.post(
            "/api/v1/currencies/1/rate",
            json={"buy_rate": 1.1, "sell_rate": 1.2},
            headers=auth_headers,
        )
...
class TestItemsExtra:
    ENDPOINTS_CREATE = [
        (
            ...
            {"name_en": "Test Cat", "code": f"TC-{uuid4().hex[:6].upper()}"},
        ),
        (
            "/api/v1/items/sub-categories",
            {"name_en": "Test Sub", "code": f"TS-{uuid4().hex[:6].upper()}"},
        ),
        (
            "/api/v1/items/master-nodes",
            {"name_en": "Test Node", "code": f"TN-{uuid4().hex[:6].upper()}"},
        ),
    ]
    ...
    async def test_create_not_500(
        self, path: str, payload: dict, client: AsyncClient, auth_headers: dict
    ):
    ...
    async def test_classify_item(self, client: AsyncClient, auth_headers: dict):
        resp = await client.post(
            "/api/v1/items/classify", json={"name": "test item"}, headers=auth_headers
        )
        assert resp.status_code < 500


class TestSuppliersExtra:
    async def test_create_supplier_not_500(
        self, client: AsyncClient, auth_headers: dict
    ):
    ...
    async def test_create_rfq_not_500(self, client: AsyncClient, auth_headers: dict):
        resp = await client.post(
            "/api/v1/suppliers/rfqs", json={}, headers=auth_headers
        )
    ...
    async def test_purchase_orders_not_500(
        self, client: AsyncClient, auth_headers: dict
    ):
        resp = await client.get(
            "/api/v1/suppliers/purchase-orders", headers=auth_headers
        )
        assert resp.status_code < 500
...
class TestCoaExtra:
    async def test_create_account_not_500(
        self, client: AsyncClient, auth_headers: dict
    ):
    ...
    async def test_create_category_not_500(
        self, client: AsyncClient, auth_headers: dict
    ):
        code = f"CAT-{uuid4().hex[:6].upper()}"
        resp = await client.post(
            "/api/v1/coa/categories",
            json={"code": code, "name_en": "Test Cat"},
            headers=auth_headers,
        )
        assert resp.status_code in (200, 201, 422, 403)
...
class TestCostMgmtExtra:
    ...
    ENDPOINTS_POST = [
        ...
        (
            "/api/v1/cost-management/budget-lines",
            {"lines": [{"category": "test", "amount": 100}]},
        ),
    ]
    ...
    async def test_execute_allocation(self, client: AsyncClient, auth_headers: dict):
        resp = await client.post(
            "/api/v1/cost-management/allocations/1/execute", headers=auth_headers
        )
        assert resp.status_code in (400, 404, 422, 403)

    async def test_sync_actuals(self, client: AsyncClient, auth_headers: dict):
        resp = await client.post(
            "/api/v1/cost-management/sync-actuals/1", headers=auth_headers
        )
...
class TestPettyCashExtra:
    ...
    async def test_approve_register(self, client: AsyncClient, auth_headers: dict):
        resp = await client.post(
            "/api/v1/petty-cash/registers/1/approve", headers=auth_headers
        )
        assert resp.status_code in (404, 422, 403)

    async def test_close_register(self, client: AsyncClient, auth_headers: dict):
        resp = await client.post(
            "/api/v1/petty-cash/registers/1/close", headers=auth_headers
        )
        assert resp.status_code in (404, 422, 403)
...
class TestProcurementExtra:
    ...
    async def test_approve_grn(self, client: AsyncClient, auth_headers: dict):
        resp = await client.post(
            "/api/v1/procurement/grn/1/approve", headers=auth_headers
        )
        assert resp.status_code in (404, 422, 403)
...
class TestStrategicExtra:
    ...
    ENDPOINTS_POST = [
        ...
        (
            "/strategic/variance",
            {"event_id": 1, "actual_cost": 1200, "budget_cost": 1000},
        ),
        ...
        (
            "/strategic/bsc/indicators",
            {"objective_id": 1, "name": "KPI 1", "unit": "%"},
        ),
        ("/strategic/bsc/measurements", {"indicator_id": 1, "value": 85.0}),
        ("/strategic/abc/pool", {"name": "Test Pool", "cost": 5000}),
        (
            "/strategic/lifecycle-cost",
            {"event_id": 1, "phase": "planning", "cost": 1000},
        ),
    ...
    async def test_calculate_profitability(
        self, client: AsyncClient, auth_headers: dict
    ):
        resp = await client.post(
            "/strategic/profitability/event/1", headers=auth_headers
        )
        assert resp.status_code < 500

    async def test_update_kaizen(self, client: AsyncClient, auth_headers: dict):
        resp = await client.put(
            "/strategic/kaizen/1/result", json={"result": 95.5}, headers=auth_headers
        )
        assert resp.status_code in (200, 404, 422, 403)
...
class TestClientsExtra:
    ...
    async def test_update_client(self, client: AsyncClient, auth_headers: dict):
        resp = await client.put(
            "/api/v1/clients/1", json={"name_en": "Updated"}, headers=auth_headers
        )
        assert resp.status_code in (200, 404, 422)
    ...
    async def test_create_client(self, client: AsyncClient, auth_headers: dict):
        ...
        resp = await client.post(
            "/api/v1/clients/",
            json={"code": code, "name_en": "Test Client"},
            headers=auth_headers,
        )
        assert resp.status_code in (200, 201, 422, 403)
...
class TestSuppliersDetail:
    ...
    async def test_update_supplier(self, client: AsyncClient, auth_headers: dict):
        resp = await client.put(
            "/api/v1/suppliers/1", json={"name_en": "Updated"}, headers=auth_headers
        )
        assert resp.status_code in (200, 404, 422)

    async def test_rate_supplier(self, client: AsyncClient, auth_headers: dict):
        resp = await client.post(
            "/api/v1/suppliers/1/rate", json={"rating": 4}, headers=auth_headers
        )
        assert resp.status_code in (200, 404, 422, 403)
    ...
    async def test_submit_quote(self, client: AsyncClient, auth_headers: dict):
        resp = await client.post(
            "/api/v1/suppliers/rfqs/1/submit-quote",
            json={"amount": 100},
            headers=auth_headers,
        )
        assert resp.status_code in (404, 422, 403)
...
class TestAccountingExtra:
    ...
    async def test_gl_post_trigger(self, client: AsyncClient, auth_headers: dict):
        resp = await client.post(
            "/api/v1/accounting/gl-post/JV/1", headers=auth_headers
        )
