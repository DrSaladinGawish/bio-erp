...
class TestATPCalculator:
    async def test_valid(self, client: AsyncClient):
        r = await client.post(
            f"{BASE}/atp", json={"substrate_mol": 10, "atp_per_mol": 36}
        )
    ...
    async def test_negative_input_returns_400(self, client: AsyncClient):
        r = await client.post(
            f"{BASE}/atp", json={"substrate_mol": -1, "atp_per_mol": 36}
        )
        assert r.status_code == 422

    async def test_zero_input(self, client: AsyncClient):
        r = await client.post(
            f"{BASE}/atp", json={"substrate_mol": 0, "atp_per_mol": 36}
        )
        assert r.status_code == 200
...
class TestEnzymeCalculator:
    async def test_valid(self, client: AsyncClient):
        r = await client.post(
            f"{BASE}/enzyme", json={"vmax": 100, "km": 5, "substrate_conc": 10}
        )
        assert r.status_code == 200
    ...
    async def test_km_zero_returns_422(self, client: AsyncClient):
        r = await client.post(
            f"{BASE}/enzyme", json={"vmax": 100, "km": 0, "substrate_conc": 10}
        )
        assert r.status_code == 422

    async def test_negative_substrate_returns_422(self, client: AsyncClient):
        r = await client.post(
            f"{BASE}/enzyme", json={"vmax": 100, "km": 5, "substrate_conc": -1}
        )
        assert r.status_code == 422
...
class TestOptimizationCalculator:
    async def test_valid_feasible(self, client: AsyncClient):
        r = await client.post(
            f"{BASE}/optimize",
            json={
                "biomass_target_gl": 10,
                "time_limit_hr": 168,
                "cost_constraint_egp": 10000,
            },
        )
        assert r.status_code == 200
        body = r.json()
    ...
    async def test_infeasible(self, client: AsyncClient):
        r = await client.post(
            f"{BASE}/optimize",
            json={
                "biomass_target_gl": 1e6,
                "time_limit_hr": 1,
                "cost_constraint_egp": 1e9,
            },
        )
        assert r.status_code == 200
...
class TestSensitivityCalculator:
    async def test_valid(self, client: AsyncClient):
        r = await client.post(
            f"{BASE}/sensitivity",
            json={"base_value": 7.0, "param_range": 2.0, "steps": 5},
        )
        assert r.status_code == 200
        body = r.json()
    ...
    async def test_min_steps(self, client: AsyncClient):
        r = await client.post(
            f"{BASE}/sensitivity",
            json={"base_value": 5.0, "param_range": 1.0, "steps": 2},
        )
        assert r.status_code == 200
    ...
    async def test_steps_below_2_returns_422(self, client: AsyncClient):
        r = await client.post(
            f"{BASE}/sensitivity",
            json={"base_value": 5.0, "param_range": 1.0, "steps": 1},
        )
        assert r.status_code == 422
...
class TestGeneCostCalculator:
    async def test_valid_iptg(self, client: AsyncClient):
        r = await client.post(
            f"{BASE}/gene-cost", json={"plasmid_size_kb": 5, "copy_number": 50}
        )
        assert r.status_code == 200
        body = r.json()
    ...
    async def test_negative_size_returns_422(self, client: AsyncClient):
        r = await client.post(
            f"{BASE}/gene-cost", json={"plasmid_size_kb": -1, "copy_number": 50}
        )
        assert r.status_code == 422
...
class TestThroughputCalculator:
    ...
    async def test_valid_kidney(self, client: AsyncClient):
        r = await client.post(
            f"{BASE}/throughput",
            json={"organ_type": "kidney", "cell_density_cells_per_ml": 5e6},
        )
        assert r.status_code == 200
