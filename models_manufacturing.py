...
class MfgProduct(CompanyTenantMixin, AuditMixin, Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "product_type IN ('RAW_MATERIAL','FINISHED_GOOD')",
    ...
    bom_lines_as_rm = relationship(
        "MfgBOMLine", back_populates="material", foreign_keys="MfgBOMLine.material_id"
    )
...
class MfgBOM(CompanyTenantMixin, AuditMixin, Base):
    ...
    id = Column(BigInteger, primary_key=True, autoincrement=True)
    product_id = Column(
        BigInteger, ForeignKey("mfg_products.id", ondelete="CASCADE"), nullable=False
    )
    is_active = Column(Boolean, nullable=False, default=True)

    finished_product = relationship(
        "MfgProduct", back_populates="bom_as_parent", foreign_keys=[product_id]
    )
    lines = relationship(
        "MfgBOMLine",
        back_populates="bom",
...
class MfgBOMLine(AuditMixin, Base):
    ...
    id = Column(BigInteger, primary_key=True, autoincrement=True)
    bom_id = Column(
        BigInteger, ForeignKey("mfg_boms.id", ondelete="CASCADE"), nullable=False
    )
    ...
    material = relationship(
        "MfgProduct", back_populates="bom_lines_as_rm", foreign_keys=[material_id]
    )
...
class MfgPORawMaterial(AuditMixin, Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "qty_issued   <= qty_planned", name="ck_mfg_po_rm_issued_limit"
        ),
        CheckConstraint(
            "qty_consumed <= qty_planned", name="ck_mfg_po_rm_consumed_limit"
        ),
    ...
    production_order = relationship(
        "MfgProductionOrder", back_populates="raw_materials"
    )
...
class MfgPOOperation(AuditMixin, Base):
    ...
    )  # operator name; nullable for PENDING
    ...
    def __repr__(self) -> str:
        return (
            f"<MfgPOOperation seq={self.op_sequence} "
...
class MfgPOTransaction(Base):
    ...
    id = Column(BigInteger, primary_key=True, autoincrement=True)
    po_id = Column(BigInteger, ForeignKey("mfg_production_orders.id"), nullable=False)
...
class MfgWarehouse(CompanyTenantMixin, AuditMixin, Base):
    ...
    locations = relationship(
        "MfgLocation", back_populates="warehouse", cascade="all, delete-orphan"
    )
...
class MfgLocation(AuditMixin, Base):
    ...
    id = Column(BigInteger, primary_key=True, autoincrement=True)
    warehouse_id = Column(
        BigInteger, ForeignKey("mfg_warehouses.id", ondelete="CASCADE"), nullable=False
    )
