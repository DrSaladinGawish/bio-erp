...
try:
    ...
    r = httpx.post(
        "http://localhost:8000/api/v1/or/decision-analysis",
        json={
            "model_name": "Test",
            "states": [{"id": "s1", "name": "Good", "probability": 0.6}],
    ...
    j = (
        r.json()
        if r.status_code == 200
        else {"sync_ready_rows": "ERROR", "permanent_gaps": []}
    )
    print(
        f"[9] Exclusions: ready={j.get('sync_ready_rows')} gaps={len(j.get('permanent_gaps', []))}"
    )
    ...
    if r.status_code == 200:
        ...
        for res in j.get("results", []):
            ...
            print(
                f"     {icon} {res['entity']}: sent={res['sent']} accepted={res['accepted']} rejected={res['rejected']}"
            )
    else:
