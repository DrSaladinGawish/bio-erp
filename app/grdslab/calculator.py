...
def min_slab_thickness_interior(P: float, k: float, fr: float, b: float) -> float:
    ...
    for _ in range(50):
        ...
        flexural_stress_interior(P, Lr, t, b)
        tn = math.sqrt(
            0.316 * P / fr * (4 * math.log10(Lr / equivalent_radius(b)) + 1.069)
        )
        if abs(tn - t) < 0.5:
            return tn
        lo = tn
...
def calculate_full(
    P_kN: float, t_mm: float, fc_MPa: float, k_kPa_mm: float, b_mm: float, Ac_mm2: float
) -> dict:
    P = P_kN * 1000
    t = t_mm
    fc = fc_MPa
