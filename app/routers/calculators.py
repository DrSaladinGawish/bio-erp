...
class BatchOptInput(BaseModel):
    ...
    doubling_time_hr: float = Field(
        24.0, gt=0, description="Cell doubling time in hours"
    )
...
class GeneCostInput(BaseModel):
    ...
    induction_method: str = Field(
        "iptg", description="Induction method (iptg, heat, auto, constitutive)"
    )


class ThroughputInput(BaseModel):
    organ_type: str = Field(
        "liver", description="Organ type (liver, kidney, heart, lung, pancreas)"
    )
