...
class TradPoolSchema(BaseModel):
    ...
    allocation_base: Literal[
        "DIRECT_LABOR_HOURS", "MACHINE_HOURS", "DIRECT_LABOR_COST", "UNITS_PRODUCED"
    ]
...
class BSCPerspectiveSchema(BaseModel):
    perspective_name: Literal[
        "FINANCIAL", "CUSTOMER", "INTERNAL_PROCESSES", "LEARNING_GROWTH"
    ]
...
class RespCenterSchema(BaseModel):
    ...
    center_type: Literal[
        "COST_CENTER", "REVENUE_CENTER", "PROFIT_CENTER", "INVESTMENT_CENTER"
    ]
...
def tdabc_product_cost(pool: TDABCPoolSchema, product: TDABCProductSchema):
    try:
        ...
        scm_module._log(
            "TDABC_PRODUCT_COST",
            {"pool": pool.model_dump(), "product": product.model_dump()},
        )
        return {
            "success": True,
...
def tdabc_idle_capacity(variance: TDABCVarianceSchema):
    try:
        pool = TDABCResourcePool(
            ...
            total_cost=variance.total_cost,
            resources_count=variance.resources_count,
            efficiency_pct=variance.efficiency_pct,
        )
        result = scm_module.tdabc_engine.calculate_idle_capacity(
            pool, variance.used_minutes
        )
...
def trad_allocate(pool: TradPoolSchema, allocation: TradAllocationSchema):
    try:
        ...
        scm_module._log(
            "TRAD_ALLOCATE",
            {"pool": pool.model_dump(), "allocation": allocation.model_dump()},
        )
        return {
            "success": True,
...
def abc_activity_rate(pool: ABCPoolSchema, allocation: ABCAllocationSchema):
    try:
        ...
        rate = scm_module.abc_rca_target_engine.calculate_activity_rate(
            p, allocation.driver_quantity
        )
...
def abc_product_cost(
    pool: ABCPoolSchema,
    allocation: ABCAllocationSchema,
    product_cost: ABCProductCostSchema,
):
    try:
        p = ABCActivityPool(**pool.model_dump())
        ...
        scm_module._log(
            "ABC_PRODUCT_COST",
            {"pool": pool.model_dump(), "product": product_cost.model_dump()},
        )
        return {
            "success": True,
            "product_name": product_cost.product_name,
            "allocated_cost": cost,
            "timestamp": datetime.now().isoformat(),
        }
    except Exception as e:
...
def target_calculate(model: TargetModelSchema):
    try:
        ...
        result = scm_module.abc_rca_target_engine.calculate_target_cost(
            m.market_price, m.target_profit_pct
        )
...
def target_cost_sheet(sheet: TargetSheetSchema):
    try:
        gap = scm_module.abc_rca_target_engine.calculate_target_gap(
            sheet.as_is_cost, sheet.target_cost
        )
        scm_module._log("TARGET_SHEET", sheet.model_dump())
        return {
            "success": True,
            "cost_component": sheet.cost_component,
            "as_is_cost": sheet.as_is_cost,
            "target_cost": sheet.target_cost,
            "gap": gap,
            "timestamp": datetime.now().isoformat(),
        }
    except Exception as e:
...
def target_summary(
    sheets: List[TargetSheetSchema],
    ve_actions: List[TargetVESchema],
    model: TargetModelSchema,
):
    try:
        m = TargetMarketModel(**model.model_dump())
        target = scm_module.abc_rca_target_engine.calculate_target_cost(
            m.market_price, m.target_profit_pct
        )
        ...
        scm_module._log(
            "TARGET_SUMMARY",
            {"model": model.model_dump(), "sheets": len(sheets), "ve": len(ve_actions)},
        )
        return {
            "success": True,
            "product_name": m.product_name,
            "target_cost": target["target_cost"],
            **result,
            "timestamp": datetime.now().isoformat(),
        }
    except Exception as e:
...
def bsc_measurement(measurement: BSCMeasurementSchema):
    try:
        vp = scm_module.bsc_resp_roi_engine.calculate_kpi_variance(
            measurement.actual_value, measurement.target_value
        )
        ...
        return {
            "success": True,
            "perspective": measurement.perspective_name,
            "variance_pct": vp,
...
def bsc_scorecard(
    measurements: List[BSCMeasurementSchema],
    perspectives: List[BSCPerspectiveSchema] = None,
):
    try:
        if perspectives is None:
            perspectives = [
                BSCPerspectiveSchema(perspective_name=p, weight_pct=25.0)
                for p in [
                    "FINANCIAL",
                    "CUSTOMER",
                    "INTERNAL_PROCESSES",
        ...
        scm_module._log(
            "BSC_SCORECARD",
            {"measurements": len(measurements), "perspectives": len(perspectives)},
        )
        return {"success": True, **result, "timestamp": datetime.now().isoformat()}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
def resp_budget_variance(budget: RespBudgetSchema):
    try:
        ...
        result = scm_module.bsc_resp_roi_engine.calculate_budget_variance(
...
def roi_eva_quick(
    nopat: float = Query(...),
    capital_employed: float = Query(...),
    wacc_pct: float = Query(...),
):
    try:
        eva = scm_module.bsc_resp_roi_engine.calculate_eva(
            nopat, capital_employed, wacc_pct
        )
        roi = scm_module.bsc_resp_roi_engine.calculate_roi(nopat, capital_employed)
        charge = scm_module.bsc_resp_roi_engine.calculate_capital_charge(
