                    ...
                    for fmt in (
                        ...
                        try:
                            parsed = datetime.strptime(
                                tx_date.split("T")[0] if "T" in tx_date else tx_date,
                                fmt,
                            )
...
class BankReimportEngine:
    ...
    def scan_excluded_transactions(
        self, source: str = "exclusion_table"
    ) -> List[Dict[str, Any]]:
        """
        ...
        if source == "exclusion_table":
            ...
            # Read from CSV
            csv_path = Path(
                r"D:\ERP System\BIO_ERP\data\excluded_bank_transactions.csv"
            )
            if csv_path.exists():
                import csv
            ...
            json_path = Path(
                r"D:\ERP System\BIO_ERP\data\excluded_bank_transactions.json"
            )
            if json_path.exists():
    ...
    def process_batch(
            ...
            if is_valid:
                ...
                if self._check_duplicate(cleaned.get("tx_hash", "")):
                    ...
                    self.warnings.append(
                        f"Duplicate hash for tx {tx.get('id', 'unknown')}"
                    )
                    continue
                ...
                valid_records.append(
                    {
                        ...
                        "reference_number": cleaned.get(
                            "reference_number", tx.get("reference_number", "")
                        ),
                        "counterparty_name": cleaned.get(
                            "counterparty_name", tx.get("counterparty_name", "")
                        ),
                        "counterparty_account": cleaned.get(
                            "counterparty_account", tx.get("counterparty_account", "")
                        ),
                        "transaction_type": cleaned.get(
                            "transaction_type", tx.get("transaction_type", "")
                        ),
                        "status": "pending_review",
                        "tx_hash": cleaned.get("tx_hash", ""),
                        "validation_warnings": json.dumps(self.validator.warnings)
        ...
        report = {
            ...
            if not dry_run
            else ["Dry run complete — no data written. Set dry_run=false to stage."],
        }
    ...
    def get_staging_status(self, batch_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "review_url": f"/api/v1/scm/staging/status?batch_id={batch_id}"
            if batch_id
            else "/api/v1/scm/staging/status",
        }

    def approve_for_deployment(
        self, record_ids: List[int], reviewer: str = "system"
    ) -> Dict[str, Any]:
...
def run_reimport(
    db: Session, source: str = "exclusion_table", dry_run: bool = True
) -> Dict[str, Any]:
