...
def intelligence_or_solve_post(
    engine: str = "all",
    payload: ORSolveIn = None,
    db: Session = Depends(get_db),
):
    params = payload.model_dump() if payload else {}
    return run_or_solver(db, engine=engine, params=params)
...
class SCMSolveIn(BaseModel):
    costs: Optional[list] = None


@router.post("/scm/analyze")
def intelligence_scm_analyze_post(
    cell: str = "all",
    payload: SCMSolveIn = None,
    db: Session = Depends(get_db),
):
    return run_scm_analysis(db, cell=cell)
