...
def analyze_value_chain(db: Session) -> dict:
    ...
    try:
        ...
        purchases = (
            db.execute(
                text("SELECT COALESCE(SUM(total_amount), 0) FROM purchase_voucher")
            ).scalar()
            or 0
        )
        sales = (
            db.execute(
                text("SELECT COALESCE(SUM(total_amount), 0) FROM sales_invoice")
            ).scalar()
            or 0
        )
        bank = (
        ...
        out.update(
            {
                ...
                "value_chain_margin_pct": round((sales - purchases) / sales * 100, 2)
                if sales
                else 0,
            }
        )
...
def analyze_strategic_cost(db: Session) -> dict:
    ...
    try:
        ...
        out["top_5_concentration_pct"] = (
            round(total_spend / sum(float(r[1]) for r in top_vendors) * 100, 1)
            if total_spend
            else 0
        )
    except Exception as exc:
