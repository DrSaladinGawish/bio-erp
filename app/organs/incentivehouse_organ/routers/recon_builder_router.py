...
async def run_smart_recon(db: Session = Depends(get_db)):
    ...
    total_var = (
        db.execute(
            text(
                "SELECT COALESCE(SUM(ABS(variance)), 0) FROM recon_matches WHERE match_status = 'VARIANCE'"
            )
        ).scalar()
        or 0.0
    )
...
async def smart_export(
    format: str = Query("csv", regex="^(csv|json)$"), db: Session = Depends(get_db)
):
    rows = db.execute(
        text("""
    ...
    filepath = EXPORTS_DIR / filename
    filepath.write_text(
        json.dumps({"exported_at": ts, "count": len(data), "records": data}, indent=2)
    )
