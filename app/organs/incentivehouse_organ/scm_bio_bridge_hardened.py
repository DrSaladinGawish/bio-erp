...
Changes from dev version:
  1. All 10 endpoints protected with require_permission()
  2. approve_promotion uses parameterized table whitelist (no f-string SQL)
  3. _guard_production_write() strengthened with table name validation
  4. SQL injection vectors eliminated via strict table whitelist
"""
...
# ── AUTH IMPORT (Bio-ERP rbac.py pattern) ──
# from app.rbac import require_permission, Permission  # Adjust path to your rbac module
# If rbac is at app/core/rbac.py, change import accordingly.
# For standalone organ dev, define a fallback:
...
try:
    ...
    try:
        ...
        # Fallback for organ dev server without full rbac wired
...
# ── SECURITY: Whitelist of allowed staging tables for promotion ──
...
# Production tables that SCM may READ
...
# Production tables that SCM may NEVER write to
PRODUCTION_WRITE_TABLES: Set[str] = set()

# Regex for safe table name validation
...
class SCMBioBridge:
    ...
    SECURITY RULES:
    1. READ from production tables only (events, clients, vendors, transactions)
    2. WRITE to scm_staging tables ONLY
    3. NEVER write to production tables from SCM directly
    4. Promotion to production requires explicit admin approval via approve_promotion()
    5. All table names validated against whitelist before SQL execution
    """
    ...
    def save_cost_analysis(self, analysis: dict) -> Dict[str, Any]:
        """Save cost analysis results to scm_staging."""
    ...
    def approve_promotion(self, request_id: int, admin_user: str) -> Dict[str, Any]:
        ...
        SQL INJECTION FIX: staging_table is validated against whitelist, never interpolated.
        """
        with self.StagingSession() as staging, self.ProdSession() as prod:
            ...
            # SECURITY: Validate table against whitelist before ANY SQL construction
            ...
            # 2. Fetch staging data — table name is now safe (whitelisted)
            # We still use text() with explicit table in the query string, but
            # the value has been validated against ALLOWED_STAGING_TABLES
...
# ═══════════════════════════════════════════════════════════════════
# FASTAPI ROUTER — ALL ENDPOINTS PROTECTED WITH require_permission
# ═════════════════════════════════════════════════════════════════==
...
# ── PRODUCTION READ ENDPOINTS (scm:read) ──
...
@router.get(
    ...
    dependencies=[
        Depends(require_permission(Permission.SCM_READ, Permission.EVENT_READ))
    ],
)
def read_events(
    ...
    """Read events from production — READ ONLY. Requires scm:read or event:read."""
    filters = {}
    if status:
...
# ── STAGING WRITE ENDPOINTS (scm:write) ──
...
def api_save_cost_analysis(
    ...
    """Save cost analysis to staging — NEVER production. Requires scm:write."""
...
def api_request_promotion(
    ...
    """Request promotion to production — requires admin approval. Requires scm:write."""
...
def api_approve_promotion(
    ...
    """Admin approves promotion — ONLY authorized path to production. Requires scm:admin."""
    return bridge.approve_promotion(request_id, admin_user)


# ── ANALYTICS ENDPOINTS (scm:read) ──
