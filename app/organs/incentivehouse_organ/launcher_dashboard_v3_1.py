...
@dataclass
class DataFlowNode:
    """A node in the data flow graph (module, table, or external system)"""
    id: str
    name: str
    type: str  # "module", "database_table", "file", "api", "external"
    module: str
    port: Optional[int]
    status: str  # "active", "idle", "error", "offline"
    last_activity: Optional[str]
    data_types: List[str]  # e.g., ["events", "transactions", "clients"]
    throughput_24h: Optional[int]  # records processed in last 24h
    error_rate: float  # percentage
    health_score: int  # 0-100

@dataclass
class DataFlowEdge:
    """An edge representing data movement between nodes"""
    id: str
    source: str  # node id
    target: str  # node id
    flow_type: str  # "sync", "async", "batch", "stream", "trigger"
    direction: str  # "one_way", "bidirectional"
    frequency: str  # "realtime", "hourly", "daily", "on_demand"
    record_count_24h: Optional[int]
    latency_ms: Optional[float]
    status: str  # "flowing", "stalled", "error", "idle"
    last_sync: Optional[str]
    schema: List[str]  # fields being transferred

@dataclass
class DataLocation:
    """Where a specific data entity lives"""
    entity_name: str  # e.g., "clients", "events", "bank_transactions"
    entity_type: str  # "master", "transactional", "reference", "staging", "audit"
    primary_module: str  # owner module
    primary_table: str
    primary_db: str
    replicas: List[Dict[str, str]]  # [{"module": "IH", "table": "clients", "sync": "realtime"}]
    file_backups: List[str]  # file paths
    api_endpoints: List[str]  # REST endpoints
    last_updated: Optional[str]
    row_count: Optional[int]
    size_mb: Optional[float]
    health_status: str  # "healthy", "stale", "orphaned", "duplicate"
    staleness_hours: Optional[float]

@dataclass
class MasterTableHealth:
    """Health status of a master/reference table"""
    table_name: str
    module: str
    database: str
    row_count: int
    row_count_trend: str  # "growing", "stable", "shrinking", "empty"
    ...
    is_stale: bool
    schema_hash: str  # detect schema drift
    schema_last_checked: str
    schema_drift_detected: bool
    drift_details: Optional[str]
    index_health: str  # "optimal", "fragmented", "missing"
...
class DataFlowSnapshot:
    """Complete snapshot of data flow state"""
    timestamp: str
...
# ═════════════════════════════════════════════════════════════════════════════
# PART 3A — DATA FLOW VISUALIZATION ENGINE
# ═════════════════════════════════════════════════════════════════════════════

class DataFlowEngine:
    """
    Tracks how data moves across the ERP ecosystem.
    Builds a directed graph of data sources, transformers, and destinations.
    """

    # Define the canonical data flow topology
    FLOW_TOPOLOGY = {
        # ── Master Data Sources ──────────────────────────────────────────────
        "node_master_clients": {
            "name": "Master Clients",
            ...
            "module": "Bio-ERP Core",
            "port": 8000,
            "data_types": ["clients", "contacts", "billing_info"],
        },
        "node_master_vendors": {
            "name": "Master Vendors",
            "type": "database_table",
            "module": "Bio-ERP Core",
            "port": 8000,
            "data_types": ["vendors", "suppliers", "contracts"],
        },
        "node_master_items": {
            "name": "Master Items/Products",
            "type": "database_table",
            "module": "Bio-ERP Core",
            "port": 8000,
            "data_types": ["items", "products", "skus"],
        },
        "node_master_coa": {
            ...
            "module": "Bio-ERP Core",
            "port": 8000,
            "data_types": ["accounts", "coa", "financial_codes"],
        },
        "node_master_staff": {
            "name": "Master Staff",
            "type": "database_table",
            "module": "Bio-ERP Core",
            "port": 8000,
            "data_types": ["employees", "staff", "roles"],
        },

        # ── EventCore Module ─────────────────────────────────────────────────
        "node_eventcore_events": {
            "name": "EventCore Events",
            "type": "database_table",
            "module": "EventCore ERP",
            "port": 8001,
            "data_types": ["events", "event_details", "schedules"],
        },
        "node_eventcore_bookings": {
            "name": "EventCore Bookings",
            "type": "database_table",
            "module": "EventCore ERP",
            "port": 8001,
            "data_types": ["bookings", "reservations", "attendees"],
        },

        # ── IncentiveHouse Module ──────────────────────────────────────────
        "node_ih_events": {
            "name": "IH Events",
            "type": "database_table",
            "module": "IncentiveHouse ERP",
            "port": 9001,
            "data_types": ["events", "event_lines", "client_events"],
        },
        "node_ih_sales": {
            "name": "IH Sales Line Items",
            "type": "database_table",
            "module": "IncentiveHouse ERP",
            "port": 9001,
            "data_types": ["sales", "line_items", "categories", "sub_categories"],
        },
        "node_ih_bank": {
            "name": "IH Bank Transactions",
            "type": "database_table",
            "module": "IncentiveHouse ERP",
            "port": 9001,
            "data_types": ["bank_transactions", "reconciliation", "ledger"],
        },
        "node_ih_clients": {
            "name": "IH Client Cache",
            "type": "database_table",
            "module": "IncentiveHouse ERP",
            "port": 9001,
            "data_types": ["clients", "client_events", "billing"],
        },

        # ── OR Module ────────────────────────────────────────────────────────
        "node_or_analysis": {
            "name": "OR Analysis Results",
            "type": "database_table",
            "module": "OR-ERP Module",
            "port": 8000,
            "data_types": ["lp_results", "eoq_results", "pert_results", "profit_analysis"],
        },
        "node_or_planning": {
            "name": "OR Planning Scenarios",
            "type": "file",
            "module": "OR-ERP Module",
            "port": 8000,
            "data_types": ["scenarios", "what_if", "recommendations"],
        },

        # ── SCM Module ───────────────────────────────────────────────────────
        "node_scm_costing": {
            "name": "SCM Costing Data",
            "type": "database_table",
            "module": "SCM Module",
            "port": None,
            "data_types": ["cost_data", "variance", "budget"],
        },
        "node_scm_staging": {
            "name": "SCM Staging Tables",
            "type": "database_table",
            "module": "SCM Module",
            "port": None,
            "data_types": ["staging", "analysis", "temp_results"],
        },

        # ── External / Files ─────────────────────────────────────────────────
        "node_excel_master": {
            "name": "Excel Master Data",
            "type": "file",
            "module": "External",
            "port": None,
            "data_types": ["coa", "items", "clients", "vendors", "staff"],
        },
        "node_excel_bank": {
            "name": "Excel Bank Transactions",
            "type": "file",
            "module": "External",
            "port": None,
            "data_types": ["bank_transactions", "narrations", "sub_ledger"],
        },
        "node_ai_ingest": {
            "name": "AI Ingest Pipeline",
            "type": "api",
            "module": "AI Layer",
            "port": None,
            "data_types": ["documents", "extracted_data", "classifications"],
        },
    }

    # Define data flow edges (who sends data to whom)
    FLOW_EDGES = [
        # Bio-ERP Master → EventCore
        ...
        # Excel → IncentiveHouse (Bank Import)
        ...
        # Excel → Bio-ERP (Master Data Load)
    ...
    def start_background_monitor(self):
        """Start background data flow monitoring"""
        self._running = True
    ...
    def capture_snapshot(self) -> DataFlowSnapshot:
        """Capture current state of all data flows"""
        start = time.time()

        # Build nodes with live status
        nodes = []
        ...
        for edge_config in self.FLOW_EDGES:
            ...
            edges.append(DataFlowEdge(
                ...
                flow_type=edge_config["flow_type"],
                direction=edge_config["direction"],
                frequency=edge_config["frequency"],
                record_count_24h=record_count,
                latency_ms=latency,
                status=edge_status,
                last_sync=self._get_last_sync(edge_config["id"]),
                schema=edge_config["schema"]
            ))

        # Get data locations
        locations = self._scan_data_locations()

        # Get master table health
    ...
    def _check_node_health(self, node_id: str, config: Dict) -> str:
        """Check if a data node is healthy"""
        port = config.get("port")
        if port:
            try:
        ...
        return "active"  # Default for API/External

    def _estimate_throughput(self, node_id: str) -> Optional[int]:
        """Estimate 24h throughput for a node"""
        estimates = {
            "node_ih_bank": 2501,
            "node_ih_sales": 190,
            "node_master_clients": 1751,
            "node_eventcore_events": 500,
        }
    ...
    def _calc_node_health(self, status: str, error_rate: float) -> int:
        ...
        base_scores = {"active": 100, "idle": 70, "error": 30, "offline": 0}
        score = base_scores.get(status, 50)
        score -= int(error_rate * 10)
        return max(0, min(100, score))

    def _check_edge_health(self, edge_config: Dict) -> str:
        """Check if data is flowing on an edge"""
        source_status = self._check_node_health(edge_config["source"], 
            self.FLOW_TOPOLOGY.get(edge_config["source"], {}))
        target_status = self._check_node_health(edge_config["target"],
            self.FLOW_TOPOLOGY.get(edge_config["target"], {}))

        if source_status == "offline" or target_status == "offline":
            return "stalled"
        if source_status == "error" or target_status == "error":
            return "error"
        if source_status == "active" and target_status in ("active", "idle"):
            return "flowing"
        return "idle"
    ...
    def _measure_edge_latency(self, edge_config: Dict) -> Optional[float]:
        ...
        flow_type_latency = {
            "sync": 50, "async": 200, "batch": 5000,
            "stream": 100, "trigger": 300
        }
        return flow_type_latency.get(edge_config["flow_type"], 100)

    def _estimate_edge_volume(self, edge_config: Dict) -> Optional[int]:
        """Estimate records transferred in last 24h"""
        return self._estimate_throughput(edge_config.get("source"))

    def _get_last_sync(self, edge_id: str) -> Optional[str]:
        """Get last sync timestamp for an edge"""
        return datetime.now().isoformat()

    def _scan_data_locations(self) -> List[DataLocation]:
        """Scan and map where all data entities live"""
        locations = []
        ...
        entity_map = [
            {
                ...
                "replicas": [
                    {"module": "IncentiveHouse ERP", "table": "clients", "sync": "realtime"},
                    {"module": "EventCore ERP", "table": "client_cache", "sync": "hourly"},
                ],
                "files": ["Data_Base_Mtbls.xlsx/Clnt_Mtbl"],
                "apis": ["/api/v1/clients", "/api/v1/clients/{id}"],
            },
            {
                "name": "events",
                "type": "transactional",
                ...
                "replicas": [
                    {"module": "IncentiveHouse ERP", "table": "events", "sync": "trigger"},
                    {"module": "Bio-ERP Core", "table": "event_sync", "sync": "realtime"},
                ],
                "files": [],
                "apis": ["/api/v1/events", "/api/v1/events/{id}"],
            },
            {
                ...
                "replicas": [
                    {"module": "Bio-ERP Core", "table": "transaction_bridge", "sync": "batch"},
                ],
            ...
            },
            {
                ...
                "replicas": [
                    {"module": "SCM Module", "table": "scm_sales_staging", "sync": "daily"},
                ],
                "files": [],
                "apis": ["/api/v1/sales/line-items", "/api/v1/jobs/{id}/line-items"],
            },
            {
                "name": "coa",
                "type": "reference",
                ...
                "replicas": [
                    {"module": "IncentiveHouse ERP", "table": "coa_cache", "sync": "on_demand"},
                ],
            ...
            },
            {
                ...
                "replicas": [
                    {"module": "IncentiveHouse ERP", "table": "items", "sync": "realtime"},
                    {"module": "EventCore ERP", "table": "item_cache", "sync": "hourly"},
                ],
            ...
            },
            {
                ...
                "replicas": [
                    {"module": "IncentiveHouse ERP", "table": "audit_log", "sync": "realtime"},
                ],
                "files": [],
                "apis": ["/api/v1/audit", "/api/v1/audit/trail"],
            },
        ]
        ...
        for entity in entity_map:
            ...
            locations.append(DataLocation(
                ...
                primary_module=entity["primary"]["module"],
                primary_table=entity["primary"]["table"],
                primary_db=entity["primary"]["db"],
                replicas=entity["replicas"],
                file_backups=entity["files"],
                api_endpoints=entity["apis"],
    ...
    def _query_entity_stats(self, entity: Dict) -> Tuple[Optional[int], Optional[str], Optional[float]]:
        """Query live stats for a data entity from database"""
        if not DB_AVAILABLE:
        ...
        table_map = {
            "clients": "clients",
            "events": "events",
            "bank_transactions": "bnk_transactions",
            "sales_line_items": "sales_line_items",
            "coa": "coa_mtble",
            "items": "einv_itm_mtble",
            "staff": "stff_mtble",
            "or_analysis_results": "or_results",
            "scm_cost_analysis": "scm_staging",
    ...
    def _check_master_tables(self) -> List[MasterTableHealth]:
        """Check health of all master/reference tables"""
        master_tables = []
        ...
        for table_def in master_table_defs:
            health = self._check_single_master_table(table_def)
            master_tables.append(health)

        return master_tables

    def _check_single_master_table(self, table_def: Dict) -> MasterTableHealth:
        """Check health of a single master table"""
        ...
        if DB_AVAILABLE:
            try:
                ...
                # Check for duplicates using MD5 hash of row content
                # FIX: Replaced invalid COUNT(DISTINCT *) with proper dedup query
                try:
                    cursor.execute(f"""
                    ...
                    # Fallback: get primary key columns and check for duplicates on them
                    try:
                        cursor.execute(f"""
                            SELECT column_name 
                        ...
                        if pk_cols:
                            pk_str = ', '.join(pk_cols)
                            cursor.execute(f"""
                                SELECT COUNT(*) - COUNT(DISTINCT ({pk_str})) FROM {table}
                            """)
                            dup_result = cursor.fetchone()
                            duplicate_count = dup_result[0] if dup_result else 0
                    except:
        ...
        return MasterTableHealth(
            ...
            schema_last_checked=datetime.now().isoformat(),
            schema_drift_detected=schema_drift,
            drift_details=drift_details,
            index_health=index_health,
            index_fragmentation_pct=index_frag,
    ...
    def _generate_flow_alerts(self, nodes, edges, locations, master_tables) -> List[str]:
        """Generate critical alerts from flow analysis"""
        alerts = []
    ...
    def get_flow_graph(self) -> Dict:
        """Get flow graph in format suitable for visualization"""
            ...
            "nodes": [
                {
                    ...
                    "throughput": n.throughput_24h,
                    "title": f"{n.name}<br>Module: {n.module}<br>Status: {n.status}<br>Health: {n.health_score}/100<br>Throughput: {n.throughput_24h or 'N/A'}"
                }
            ...
            "edges": [
                {
                    ...
                    "volume": e.record_count_24h,
                    "title": f"{e.flow_type.upper()}<br>Status: {e.status}<br>Latency: {e.latency_ms}ms<br>Volume: {e.record_count_24h or 'N/A'}"
                }
...
app = FastAPI(title="ERP IH Launcher — Part 3", version="3.1.0", lifespan=lifespan)
...
DASHBOARD_HTML_PATH = HERE / "dashboard_part3.html"

def _load_dashboard_html() -> str:
    """Load dashboard HTML from external file (no triple-quote conflicts)."""
    if DASHBOARD_HTML_PATH.exists():
        return DASHBOARD_HTML_PATH.read_text(encoding="utf-8")
    # Fallback: try project root
    root = HERE.parent.parent.parent / "dashboard_part3.html"
    if root.exists():
        return root.read_text(encoding="utf-8")
    return "<html><body><h1>ERP IH Launcher Part 3</h1><p>dashboard_part3.html not found next to this script.</p></body></html>"
...
async def _ws_broadcast_loop():
    """Broadcast snapshot updates to all connected WebSocket clients."""
    while True:
...
async def launcher_status():
    """Launcher health and status endpoint."""
    snapshot = flow_engine.get_snapshot()
    return {
        "launcher": "ERP IH Launcher Part 3",
        "version": "3.1.0",
        "port": 9003,
        "status": "running",
...
async def get_flow_snapshot():
    """Get complete data flow snapshot"""
    snapshot = flow_engine.get_snapshot()
    if not snapshot:
...
@app.post("/api/v1/flow/snapshot")
async def post_flow_snapshot(background_tasks: BackgroundTasks):
    """Trigger a new snapshot capture."""
...
async def get_flow_graph():
    """Get data flow graph for visualization"""
    return flow_engine.get_flow_graph()

@app.get("/api/v1/flow/nodes")
async def get_flow_nodes():
    """Get all data flow nodes"""
    snapshot = flow_engine.get_snapshot()
    if not snapshot:
        return {"nodes": []}
    return {"nodes": [asdict(n) for n in snapshot.nodes]}
...
async def get_flow_edges():
    ...
    if not snapshot:
        return {"edges": []}
    return {"edges": [asdict(e) for e in snapshot.edges]}
...
# ── API Endpoints — Data Locations ───────────────────────────────────────────

@app.get("/api/v1/locations")
async def get_data_locations(
    entity_type: Optional[str] = Query(None, description="Filter by type: master, transactional, reference, staging, audit"),
    health_status: Optional[str] = Query(None, description="Filter by health: healthy, stale, orphaned, duplicate")
):
    """Get all data locations with optional filtering"""
    snapshot = flow_engine.get_snapshot()
    if not snapshot:
        return {"locations": []}
...
async def get_data_location(entity_name: str):
    """Get specific data entity location details"""
    snapshot = flow_engine.get_snapshot()
    if not snapshot:
        raise HTTPException(404, "No snapshot")
...
async def get_data_location_map():
    """Get complete data location map grouped by module"""
    snapshot = flow_engine.get_snapshot()
    if not snapshot:
        return {"map": {}}
    ...
    for loc in snapshot.locations:
        by_module[loc.primary_module].append({
            "entity": loc.entity_name,
            "type": loc.entity_type,
            "table": loc.primary_table,
            "db": loc.primary_db,
            "rows": loc.row_count,
            "health": loc.health_status,
            "stale_hours": loc.staleness_hours,
            "replicas": len(loc.replicas),
            "files": len(loc.file_backups),
            "apis": len(loc.api_endpoints)
        })

    return {"map": dict(by_module), "total_entities": len(snapshot.locations)}

# ── API Endpoints — Master Table Health ──────────────────────────────────────
...
async def get_master_tables(
    ...
    """Get master table health status with filtering"""
    snapshot = flow_engine.get_snapshot()
    if not snapshot:
        return {"tables": []}
...
async def get_master_table(table_name: str):
    """Get specific master table health details"""
    snapshot = flow_engine.get_snapshot()
    if not snapshot:
        raise HTTPException(404, "No snapshot")
...
async def get_master_tables_summary():
    """Get summary statistics for all master tables"""
    snapshot = flow_engine.get_snapshot()
    if not snapshot:
    ...
    return {
        ...
        "by_module": {
            module: {
                ...
                "avg_health": round(sum(t.health_score for t in tables if t.module == module) / 
                                   max(len([t for t in tables if t.module == module]), 1), 1)
            }
...
async def refresh_master_table(table_name: str):
    """Trigger manual refresh check for a master table"""
    return {
...
if __name__ == "__main__":
    print("=" * 70)
    print("  ERP IH Launcher — Part 3: Data Flow & Health Monitor")
