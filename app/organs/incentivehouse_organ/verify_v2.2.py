...
m = req(
    "POST", "/v2/extract/master", {"source_file": "Data_Base_Mtbls.xlsx"}, token=token
)
...
results.append(
    (
        "1b Master",
        "POST /v2/extract/master",
        "200" if m_ok else "FAIL",
        f"{m_recs} records",
    )
)
print(
    f"MASTER: status={m.get('status')} tables={m.get('tables_processed')} records={m_recs}"
)
...
e1 = req(
    "POST",
    "/v2/extract",
    {"module": "BNK", "source_file": "Bnk_TRNX SOURCE.xlsx", "dry_run": False},
    token=token,
)
e1_ok = e1.get("status") in ("SUCCESS", "PARTIAL")
e1_recs = (
    e1.get("records_extracted")
    or e1.get("records_inserted")
    or e1.get("records_read")
    or 0
)
results.append(
    (
        "1 Extract",
        "POST /v2/extract BNK",
        "200" if e1_ok else "FAIL",
        f"{e1_recs} records",
    )
)
print(
    f"EXTRACT BNK: status={e1.get('status')} response={json.dumps(e1, indent=2)[:300]}"
)
...
results.append(
    (
        "2 Validate",
        "POST /v2/validate",
        "200" if v_ok else "FAIL",
        f"qscore={v.get('quality_score')}",
    )
)
...
s = req(
    "POST",
    "/v2/stage",
    {"validate_id": vid, "target_table": "bnk_staging"},
    token=token,
)
...
results.append(
    (
        "3 Stage",
        "POST /v2/stage",
        "200" if s_ok else "FAIL",
        f"snap={s.get('snapshot_id', '')[:20]}",
    )
)
...
results.append(
    ("4 Reconcile", "POST /v2/reconcile", "200" if rc_ok else "FAIL", f"rid={rc_id}")
)
print(f"RECONCILE: status={rc.get('status')} response={json.dumps(rc, indent=2)[:200]}")
rid = rc_id

ap = req(
    "POST", "/v2/approve", {"recon_id": rid, "approval_level": "auto"}, token=token
)
...
results.append(
    (
        "5 Approve",
        "POST /v2/approve",
        "200" if ap_ok else "FAIL",
        f"auto={ap.get('auto_approved')}",
    )
)
...
results.append(
    (
        "6 Promote",
        "POST /v2/promote",
        "200" if pr_ok else "FAIL",
        f"rb={pr.get('rollback_token', '')[:25]}",
    )
)
...
results.append(
    (
        "7 Observe",
        "POST /v2/observe",
        "200" if ob_ok else "FAIL",
        f"alerts={ob.get('alert_count')}",
    )
)
...
results.append(
    ("Status", "GET /v2/status", "200" if st_ok else "FAIL", f"{total_recs} total")
)
