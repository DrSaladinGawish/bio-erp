...
from app.organs.or_organ.or_erp_module import ORERPModule, InventoryItem
...
class EventORBridge:
    ...
    def __init__(
        ...
        sandbox_dir: str = "./analysis_sandbox",
    ):
        self.db = db
    ...
    def _run_lp(self, event: Dict, budget_lines: List[Dict]) -> Dict:
        ...
        coefficients = [
            bl.get("selling_price", 0) - bl.get("unit_cost", 0) for bl in budget_lines
        ]
        ...
        constraints = [
            {
                "coefficients": [
                    1.0 if i == idx else 0.0 for i in range(len(budget_lines))
                ],
                "rhs": float(rhs),
                "operator": "<=",
            }
    ...
    def _run_pert(self, event: Dict, line_items: List[Dict]) -> Dict:
        ...
        for li in line_items:
            ...
            section_durations[section] = max(
                section_durations.get(section, 0), duration
            )
    ...
    def _run_cvp(self, event: Dict) -> Dict:
        ...
        result = self.or_module.analyze_cost_profit(
            fixed_costs, variable_cost, selling_price, target_profit=0
        )
        ...
        return _sanitize(
            {
                "status": "completed",
                "breakeven_units": result.get("basic_analysis", {}).get(
                    "break_even_units"
                ),
                "breakeven_revenue": result.get("basic_analysis", {}).get(
                    "break_even_revenue"
                ),
                "profit": profit_amount,
                "margin_pct": margin_pct,
                "recommendation": "Excellent margin"
                if margin_pct > 20
                else "Good margin"
                if margin_pct > 10
    ...
    def _generate_recommendations(
        self, lp: Dict, eoq: Dict, pert: Dict, cvp: Dict
    ) -> List[Dict]:
        recs = []
        ...
        if lp.get("status") == "failed":
            recs.append(
                {
                    "type": "warning",
                    "message": "Resources may be insufficient — consider outsourcing",
                }
            )
        ...
        if pert.get("status") == "completed":
            ...
            if dur and dur > 30:
                recs.append(
                    {
                        "type": "warning",
                        "message": f"Project duration {dur}d is long — review critical path",
                    }
                )
        ...
        if cvp.get("status") == "completed":
            ...
            if margin < 0:
                recs.append(
                    {
                        "type": "critical",
                        "message": "Event is projected to lose money — restructure pricing",
                    }
                )
            elif margin < 10:
                recs.append(
                    {
                        "type": "warning",
                        "message": f"Margin {margin:.1f}% is thin — review costs",
                    }
                )
            else:
                recs.append(
                    {"type": "success", "message": f"Healthy margin at {margin:.1f}%"}
                )
        ...
        result = _sanitize(
            {
                ...
                "overall_feasible": all(
                    r.get("type") != "critical" for r in recommendations
                ),
            }
        )
