...
class SurgeryProtocol:
    ...
    async def _write_audit(
        ...
        log = SurgeryAuditLog(
            ...
            protocol="surgery",
            action=action,
            status=SurgeryStatus.committed
            if status == "success"
            else SurgeryStatus.failed,
            table_name=table,
            record_id=row_id,
    ...
    async def pre_op_snapshot(
        self, table_name: str, row_id: int
    ) -> SurgicalSnapshot | None:
        try:
            ...
            if row:
                data = dict(row)
                snap = SurgicalSnapshot(
                    table_name=table_name,
                    row_id=row_id,
                    column_data=copy.deepcopy(data),
                )
            ...
            await self._write_audit(
                SurgeryStage.PRE_OP, table_name, row_id, "snapshot", "failed", str(e)
            )
        return None
    ...
    async def incision(
        ...
        try:
            ...
            if verify_sql:
                ...
                if not vr.scalar():
                    raise RuntimeError(
                        "Suture check failed: verify_sql returned no result"
                    )
            ...
            snap = next(
                (
                    ...
                    if s.table_name == table_name and s.row_id == row_id
                ),
                None,
            )
            if snap:
            ...
            await self._write_audit(
                SurgeryStage.INCISION, table_name, row_id, action, "failed", str(e)
            )
    ...
    async def suture(
        ...
        table_name: str,
        row_id: int | None,
        validation_fn: Callable[[AsyncSession], bool],
    ) -> bool:
        try:
            ...
            if valid:
                ...
                await self.recovery(rollback=True)
                raise RuntimeError(
                    f"Suture validation failed for {table_name}.{row_id}"
                )
            return valid
        except Exception as e:
            await self._write_audit(
                SurgeryStage.SUTURE, table_name, row_id, "validation", "failed", str(e)
            )
            await self.recovery(rollback=True)
            raise
...
async def surgical_post_transaction(
    ...
    async def do_insert():
        ...
        if not lines and suggestion.extracted_entities:
            entities = (
                suggestion.extracted_entities
                if isinstance(suggestion.extracted_entities, dict)
                else {}
            )
        ...
        for i, line in enumerate(lines, start=1):
            ...
            if gl_account_id > 0:
                chk = await db.execute(
                    text("SELECT id FROM coa_accounts WHERE id = :aid"),
                    {"aid": gl_account_id},
                )
            ...
            if gl_account_id <= 0:
                fallback = await db.execute(
                    text(
                        "SELECT id FROM coa_accounts WHERE is_active = TRUE ORDER BY id LIMIT 1"
                    )
                )
            ...
            line_desc = (line.get("description") or line.get("desc") or description)[
                :255
            ]
    ...
    posted_id = await sp.incision(
        "jv_headers",
        None,
        "insert_jv",
        do_insert,
        verify_sql="SELECT id FROM jv_headers WHERE id = (SELECT MAX(id) FROM jv_headers)",
    )

    async def validate(s):
        row = await s.execute(
            text("SELECT id, total_debit FROM jv_headers WHERE id = :id"),
            {"id": posted_id},
        )
    ...
    async def update_sug():
        suggestion.status = (
            SuggestionStatus.amended if amended else SuggestionStatus.approved
        )
    ...
    await sp.incision(
        "ai_suggested_transaction", suggestion.id, "update_status", update_sug
    )
