...
try:
    ...
    results.append(
        ("Bio-ERP health", f"{s} {j['status']} v{j['version']} db={j['database']}")
    )
    ...
    try:
        ...
        for res in j.get("results", []):
            print(
                f"  {res['entity']}: sent={res['sent']} acc={res['accepted']} rej={res['rejected']}"
            )
        results.append(
            ("LIVE SYNC", f"batch={j.get('batch_id')} online={j.get('bio_erp_online')}")
        )
    except urllib.error.HTTPError as e:
