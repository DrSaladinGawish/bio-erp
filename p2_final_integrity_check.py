...
try:
    spec = importlib.util.spec_from_file_location(
        "prescription_receiver", f"{EC_PATH}\app\routers\prescription_receiver.py"
    )
...
try:
    spec = importlib.util.spec_from_file_location(
        "prescription_sender", f"{BIO_PATH}\app\or_module\prescription_sender.py"
    )
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
...
try:
    ...
    ec_routes = [
        r.path
        for r in ec_main.app.routes
        if hasattr(r, "path") and "prescription" in r.path
    ]
    expected_ec = [
        "/api/v1/prescriptions/receive",
        "/api/v1/prescriptions/job/{job_id}",
        "/api/v1/prescriptions/{prescription_id}/status",
    ]
...
# 4.2 Auth & End-to-End
print("")
print("[4.2] Auth + End-to-End Prescription Push...")
try:
    # Login
    r = requests.post(
        f"{BIO_BASE}/api/v1/auth/login",
    ...
    if r.status_code != 200:
        ...
        r2 = requests.post(
            f"{BIO_BASE}/api/v1/or/prescriptions/send",
            headers=headers,
            json={"job_id": 1, "analysis_types": ["lp", "pert", "tdabc"]},
            timeout=15,
        )
        if r2.status_code == 200:
            ...
            if r3.status_code == 200:
                ...
                if rx:
                    r4 = requests.patch(
                        f"{EC_BASE}/api/v1/prescriptions/{rx[0]['id']}/status?status=applied",
                        timeout=10,
                    )
                    if r4.status_code == 200:
                        ...
                        log(
                            "E2E: Status Update", "FAIL", f"{r4.status_code}: {r4.text}"
                        )
            else:
...
if failed == 0:
    print("")
    print("P2 REVERSE BRIDGE — FULLY VERIFIED")
    print("Ready for P4 (Auto-Trigger Engine)")
else:
    print("")
    print(f"{failed} check(s) failed. Fix before proceeding to P4.")
    for r in results:
        if r["status"] == "FAIL":
